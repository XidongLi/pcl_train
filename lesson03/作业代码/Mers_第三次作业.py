...
class K_Means(object):
    ...
    def __init__(self ,data,n_clusters=2, tolerance=0.000001, max_iter=300):
        self.k_ = n_clusters
        self.tolerance_ = tolerance
        self.max_iter_ = max_iter
        self.avr=np.zeros((n_clusters, 2))#中心点
        self.avr_id=np.arange(n_clusters)#类标签
        for i in range(n_clusters):
            self.avr[i,:]=data[0][random.randint(0, 1499),:]
        print(self.avr)
    def fit(self, data):#传入单个点
        # 作业1
        # 屏蔽开始
        dist=10000
        i=-1
    ...
    def predict(self, p_datas):#传入整个数据
...
while a==1:
    for i,point in enumerate(noisy_circles[0]):
        noisy_circles[1][i]=test.fit(point)
    a=test.predict(noisy_circles)
...
while a==1:
    for i,point in enumerate(noisy_moons[0]):
        noisy_moons[1][i]=test.fit(point)
    a=test.predict(noisy_moons)
...
while a==1:
    for i,point in enumerate(blobs[0]):
        blobs[1][i]=test.fit(point)
    a=test.predict(blobs)
plt.scatter(blobs[0][:,0],blobs[0][:,1],c=blobs[1])
plt.show()
...
while a==1:
    for i,point in enumerate(no_structure[0]):
        no_structure[1][i]=test.fit(point)
    a=test.predict(no_structure)
plt.scatter(no_structure[0][:,0],no_structure[0][:,1],c=no_structure[1])
plt.show()
...
while a==1:
    for i,point in enumerate(aniso[0]):
        aniso[1][i]=test.fit(point)
    a=test.predict(aniso)
plt.scatter(aniso[0][:,0],aniso[0][:,1],c=aniso[1])
plt.show()
...
while a==1:
    for i,point in enumerate(varied[0]):
        varied[1][i]=test.fit(point)
    a=test.predict(varied)
plt.scatter(varied[0][:,0],varied[0][:,1],c=varied[1])
plt.show()
